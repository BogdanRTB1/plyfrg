...
import { X, TrendingUp, Trophy, Goal, Plane, Minus, Plus, MoreHorizontal } from "lucide-react";
    ...
    return createPortal(
        <div className="fixed inset-0 z-[100] flex items-stretch md:items-center justify-center p-0 md:p-4 overflow-hidden bg-black md:bg-black/80 backdrop-blur-none md:backdrop-blur-sm">
            <motion.div
                ...
                className={`${AVIATOR_CONFIG.theme.background} rounded-none md:rounded-2xl w-full max-w-5xl border border-white/10 shadow-2xl overflow-hidden flex flex-col-reverse md:flex-row h-[100dvh] max-h-[100dvh] md:h-[700px] md:max-h-[90vh] min-h-0`}
            >
                <MobileGameHudBar
                    className={AVIATOR_CONFIG.theme.panelBg}
                    left={
                        <>
                            <button type="button" disabled={gameState === "PLAYING"} onClick={() => handleBetChange(betAmount / 2)} className="shrink-0 rounded-lg border border-white/10 bg-[#381a57] px-2.5 py-2.5 text-[11px] font-black text-slate-200 active:scale-95 disabled:opacity-40">½</button>
                            <button type="button" disabled={gameState === "PLAYING"} onClick={() => handleBetChange(betAmount * 2)} className="shrink-0 rounded-lg border border-white/10 bg-[#381a57] px-2.5 py-2.5 text-[11px] font-black text-slate-200 active:scale-95 disabled:opacity-40">2×</button>
                            <div className="flex min-w-0 max-w-[5.5rem] items-center overflow-hidden rounded-lg border border-white/10 bg-[#110521]">
                    ...
                    center={
                        gameState === "PLAYING" && cashedOutAt === null ? (
                            ...
                            <button
                                ...
                                className={`flex h-[68px] w-[68px] items-center justify-center rounded-full ${AVIATOR_CONFIG.theme.buttonAccent} text-white shadow-[0_0_22px_rgba(168,85,247,0.35)] active:scale-95 disabled:cursor-not-allowed disabled:opacity-45`}
                                aria-label="Place bet"
                            >
                                <Plane className="h-6 w-6" strokeWidth={2.2} />
                            </button>
                        )
                    }
                    right={
                        <>
                            <button type="button" disabled={gameState === "PLAYING"} onClick={() => handleBetChange(balance)} className={`shrink-0 rounded-lg border border-purple-500/30 bg-[#381a57] px-2.5 py-2.5 text-[11px] font-black ${AVIATOR_CONFIG.theme.accent} active:scale-95 disabled:opacity-40`}>MAX</button>
                            ...
                            <button type="button" onClick={() => setMobileMoreOpen(true)} className="shrink-0 rounded-lg border border-white/10 bg-[#381a57] p-2 text-slate-300 active:bg-white/10" aria-label="More options"><MoreHorizontal className="h-4 w-4" /></button>
                        </>
                    }
                />

                {/* ADVANCED BETTING MENU — desktop */}
                <div className={`hidden md:flex md:w-80 md:max-h-none md:shrink-0 md:overflow-y-auto md:overscroll-contain ${AVIATOR_CONFIG.theme.panelBg} flex-col gap-2 border-r border-black/50 p-3 md:p-6 md:gap-4 z-20`}>
                ...
                {/* GAME AREA */}
                <div className={`flex-1 relative min-h-[48vh] md:min-h-0 ${AVIATOR_CONFIG.theme.graphBg} bg-cover bg-center p-2 sm:p-6 flex flex-col justify-center overflow-hidden shadow-inner`}>
            ...
            <AnimatePresence>
                {mobileMoreOpen && (
                    <motion.div key="aviator-mobile-more" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[110] flex flex-col justify-end md:hidden">
                        <button type="button" className="min-h-0 flex-1 bg-black/55" aria-label="Close menu" onClick={() => setMobileMoreOpen(false)} />
                        <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} transition={{ type: "spring", damping: 28, stiffness: 320 }} className="max-h-[min(70vh,520px)] overflow-y-auto overscroll-contain rounded-t-2xl border border-white/10 border-b-0 bg-[#25103a] px-4 pb-[max(1rem,env(safe-area-inset-bottom))] pt-3 shadow-2xl" onClick={(e) => e.stopPropagation()}>
                            <div className="mx-auto mb-3 h-1 w-10 rounded-full bg-white/20" />
                            <div className="mb-4 flex items-center justify-between">
                                <span className="text-xs font-bold uppercase text-slate-500">{AVIATOR_CONFIG.names.title}</span>
                                <FavoriteToggle gameName={AVIATOR_CONFIG.names.title} />
                            </div>
                            <div className="mb-4 space-y-2">
                                ...
                                <div className="relative">
                                    ...
                                    <input type="number" step="0.01" min="1.01" value={autoCashout} onChange={(e) => setAutoCashout(e.target.value)} disabled={gameState === "PLAYING"} placeholder="Off" className="w-full rounded-xl border border-white/10 bg-[#110521] py-3 pl-10 pr-8 font-mono text-base font-bold text-white outline-none focus:border-purple-500 disabled:opacity-50" />
                                    <span className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 font-bold text-slate-500">×</span>
                                </div>
                            </div>
                            <div className="mb-4 flex items-center justify-between rounded-xl border border-green-500/20 bg-[#110521]/50 p-3">
                                <div className="flex items-center gap-2">
                            ...
                            <button type="button" onClick={() => setMobileMoreOpen(false)} className="w-full rounded-xl border border-white/10 bg-[#381a57] py-3 text-sm font-bold text-white active:bg-white/10">
                                Done
                            </button>
                        </motion.div>
                    </motion.div>
