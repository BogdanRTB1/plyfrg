...
export default function CustomMinesModal({ isOpen, onClose, gameData, diamonds, setDiamonds, forgesCoins, setForgesCoins }: any) {
    ...
    const MINE_IMAGE = gameData?.config?.mineImage;
    const GEM_IMAGE = gameData?.config?.gemImage;
    ...
    return createPortal(
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm">
            <motion.div
                ...
                className={`bg-[#0f212e] rounded-2xl w-full max-w-4xl border border-orange-500/30 shadow-[0_0_50px_rgba(249,115,22,0.15)] overflow-hidden flex flex-col md:flex-row h-[600px]`}
            >
                ...
                <div className={`w-full md:w-80 bg-[#121c22] p-6 flex flex-col gap-4 border-r border-white/5 z-20`}>
                    <div className="flex justify-between items-center mb-2">
                        <div className="flex flex-col gap-1 text-white">
                            ...
                            <p className="text-[10px] text-orange-400 font-bold uppercase tracking-widest truncate w-[200px]">By {gameData.creatorName}</p>
                        </div>
                    ...
                    <div className="space-y-2 mt-2">
                        ...
                        <div className="relative group">
                            ...
                            <input
                                ...
                                className="w-full bg-[#0a1114] border border-white/10 focus:border-orange-500 focus:shadow-[0_0_10px_rgba(249,115,22,0.2)] rounded-xl py-3 pl-10 pr-4 text-white font-mono text-lg font-bold transition-all outline-none"
                            />
                        </div>
                        <div className="grid grid-cols-4 gap-2 pb-2">
                            ...
                            <button onClick={() => handleBetChange(balance)} disabled={gameState === 'PLAYING'} className="bg-[#1a2c38] hover:bg-[#2f4553] text-orange-500 text-xs font-black py-2 rounded-lg border border-orange-500/30 transition-colors">MAX</button>
                        </div>
                        ...
                        <select
                            ...
                            disabled={gameState === 'PLAYING'}
                            className="w-full bg-[#0a1114] border border-white/10 focus:border-orange-500 rounded-xl py-3 px-4 text-white font-bold outline-none appearance-none"
                        >
                    ...
                    <div className="mt-auto">
                        {gameState !== 'PLAYING' ? (
                            <button onClick={startGame} disabled={balance < betAmount || betAmount <= 0} className={`w-full bg-gradient-to-r from-orange-500 to-amber-500 hover:brightness-110 text-white h-14 rounded-xl font-black text-lg tracking-widest uppercase transition-all shadow-[0_5px_20px_rgba(249,115,22,0.3)] disabled:opacity-50 disabled:cursor-not-allowed hover:-translate-y-1 relative overflow-hidden group`}>
                ...
                <div className="flex-1 relative bg-[#06090c] p-4 flex flex-col items-center justify-center overflow-hidden">
                    <div className="absolute inset-x-0 top-0 p-6 flex justify-between items-center z-10 w-full max-w-[500px] mx-auto">
                        <div className="bg-[#121c22] border border-white/5 rounded-xl px-4 py-2 flex items-center gap-3 shadow-lg">
                            ...
                            <span className="text-orange-500 font-mono font-black text-xl">{multiplier.toFixed(2)}x</span>
                        </div>
                        {gameState === 'PLAYING' && (
                            <div className="bg-[#121c22] border border-white/5 rounded-xl px-4 py-2 flex items-center gap-3 shadow-lg">
                    ...
                    <div className={`grid gap-2 sm:gap-3 lg:gap-4 p-4 mt-8`} style={{ gridTemplateColumns: `repeat(5, minmax(0, 1fr))` }}>
                        {grid.length === 25 ? grid.map((cell, idx) => (
                            <button
                                ...
                                className={`
                                    ...
                                    ${!cell.revealed ? 'bg-[#1a2c38] hover:bg-white/10 hover:scale-105 shadow-md hover:shadow-xl cursor-pointer' : ''}
                            ...
                            Array(25).fill(0).map((_, idx) => (
                                <div key={idx} className={`w-12 h-12 sm:w-16 sm:h-16 lg:w-20 lg:h-20 rounded-xl bg-[#1a2c38] opacity-50`}></div>
                            ))
                        )}
                    </div>
                </div>
