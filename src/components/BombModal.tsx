...
import { X, Scissors, Trophy, Bomb, ShieldAlert, Minus, Plus, MoreHorizontal } from "lucide-react";
    ...
    return createPortal(
        <div className="fixed inset-0 z-[100] flex items-stretch md:items-center justify-center p-0 md:p-4 overflow-hidden bg-black md:bg-black/80 backdrop-blur-none md:backdrop-blur-sm">
            <motion.div
                ...
                className="bg-[#0f212e] rounded-none md:rounded-2xl w-full max-w-4xl border border-red-500/20 shadow-[0_0_50px_rgba(239,68,68,0.1)] overflow-hidden flex flex-col-reverse md:flex-row h-[100dvh] max-h-[100dvh] md:h-[600px] md:max-h-[90vh] min-h-0"
            >
                <MobileGameHudBar
                    className="bg-[#121c22]"
                    left={
                    ...
                    center={
                        gameState === "PLAYING" ? (
                            <button type="button" onClick={cashOut} className="flex h-[68px] w-[68px] flex-col items-center justify-center rounded-full bg-gradient-to-br from-green-500 to-emerald-600 text-[10px] font-black uppercase leading-tight text-white shadow-[0_0_20px_rgba(34,197,94,0.45)] active:scale-95">
                                <Scissors className="mb-0.5 h-5 w-5" />
                                <span className="text-[10px]">{(betAmount * multiplier).toFixed(0)}</span>
                            </button>
                        ) : (
                            <button type="button" onClick={startGame} disabled={balance < betAmount || betAmount <= 0} className="flex h-[68px] w-[68px] items-center justify-center rounded-full bg-gradient-to-br from-red-600 to-red-500 text-white shadow-[0_0_22px_rgba(239,68,68,0.35)] active:scale-95 disabled:cursor-not-allowed disabled:opacity-45" aria-label="Arm bomb">
                    ...
                    right={
                        <>
                            <button type="button" disabled={gameState === "PLAYING"} onClick={() => handleBetChange(balance)} className="shrink-0 rounded-lg border border-red-500/30 bg-[#1a2c38] px-3 py-3 text-xs font-black text-red-400 active:scale-95 disabled:opacity-40">MAX</button>
            ...
            <AnimatePresence>
                {mobileMoreOpen && (
                    <motion.div key="bomb-mobile-more" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[110] flex flex-col justify-end md:hidden">
