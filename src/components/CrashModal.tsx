...
import { X, TrendingUp, Zap, Trophy, Goal, MoreHorizontal } from "lucide-react";
...
import { calcOriginalsWin, generateCrashPoint, CRASH_MIN_CASHOUT_MULT, CRASH_ROUND_COOLDOWN_MS, canCrashCashOut } from "@/utils/originalsMath";
...
export default function CrashModal({ isOpen, onClose, diamonds, setDiamonds, forgesCoins, setForgesCoins }: any) {
    ...
    const updateGame = (time: number) => {
        ...
        // Check auto cashout (respects min multiplier + min play time)
    ...
    return createPortal(
        <div className="fixed inset-0 z-[100] flex items-stretch md:items-center justify-center p-0 md:p-4 overflow-hidden bg-black md:bg-black/80 backdrop-blur-none md:backdrop-blur-sm">
                ...
                <MobileGameHudBar
                    ...
                    left={
                        <MobileHudBetRow
                            ...
                            inputClassName="min-h-[40px] min-w-[3rem] flex-1 basis-0 max-w-[6.75rem] rounded-lg border border-white/10 bg-[#0a1114] px-1 py-1 text-center text-[11px] font-mono font-bold text-white outline-none focus:border-[#00b9f0]/50 disabled:opacity-40"
                        />
                    }
                    center={
                        gameState === "PLAYING" && cashedOutAt === null ? (
                            <button
                                ...
                                onClick={() => cashOut()}
                                disabled={!canCashOut}
                                className={`flex h-[68px] w-[68px] flex-col items-center justify-center rounded-full text-[10px] font-black uppercase leading-tight text-[#0f212e] active:scale-95 ${canCashOut ? "bg-green-500 shadow-[0_0_20px_rgba(34,197,94,0.45)]" : "cursor-not-allowed bg-slate-600 opacity-50"}`}
                            >
                                <span className="text-[9px]">{canCashOut ? "Out" : "…"}</span>
                                <span className="text-xs font-black">{canCashOut ? (betAmount * multiplier).toFixed(0) : "Hold"}</span>
                            </button>
                        ) : (
                            <button
                                type="button"
            ...
            <GameLeaderboardModal isOpen={leaderboardOpen} onClose={() => setLeaderboardOpen(false)} gameName={"Crash"} />
            </AnimatePresence>
        </div>,
        document.body
    );
