...
export const SLOTS_CONFIG = {
    theme: {
        ...
        panelBg: "bg-[#121c22]",
        accent: "text-[#00b9f0]",
        buttonAccent: "bg-gradient-to-r from-[#00b9f0] to-[#38bdf8]",
        slotBg: "bg-[#06090c]",
        reelBg: "bg-[#121c22]",
    },
    names: {
    ...
    symbols: [
        ...
        { id: 5, icon: "💎", multiplier: 25 },
        { id: 6, icon: "7️⃣", multiplier: 100 }
    ]
};

export default function SlotsModal({ isOpen, onClose, diamonds, setDiamonds, forgesCoins, setForgesCoins }: any) {
    ...
    const spinReels = () => {
        ...
        setTimeout(() => {
            ...
            // Check win condition (all 3 match)
            if (newReels[0] === newReels[1] && newReels[1] === newReels[2]) {
                const multi = SLOTS_CONFIG.symbols[newReels[0]].multiplier;
    ...
    return createPortal(
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm">
            <motion.div
                ...
                className={`${SLOTS_CONFIG.theme.background} rounded-2xl w-full max-w-4xl border border-white/10 shadow-2xl overflow-hidden flex flex-col md:flex-row h-[600px]`}
            >
                {/* ADVANCED BETTING MENU */}
                <div className={`w-full md:w-80 ${SLOTS_CONFIG.theme.panelBg} p-6 flex flex-col gap-4 border-r border-white/5 z-20`}>
                    ...
                    <div className="bg-[#0a1114]/50 border border-white/5 rounded-xl p-3 flex flex-col gap-2 mt-2">
                        ...
                        <div className="grid grid-cols-2 gap-2 max-h-24 overflow-y-auto custom-scrollbar pr-1">
                            {SLOTS_CONFIG.symbols.map(sym => (
                                <div key={sym.id} className="flex justify-between items-center bg-[#121c22] rounded px-2 py-1">
                    ...
                    <div className="mt-auto">
                        <button onClick={spinReels} disabled={balance < betAmount || betAmount <= 0 || gameState === 'SPINNING'} className={`w-full ${SLOTS_CONFIG.theme.buttonAccent} hover:brightness-110 text-[#0f212e] h-14 rounded-xl font-black text-xl tracking-widest uppercase transition-all shadow-[0_5px_20px_rgba(0,185,240,0.3)] disabled:opacity-50 disabled:cursor-not-allowed hover:-translate-y-1 relative overflow-hidden group`}>
                ...
                <div className={`flex-1 relative ${SLOTS_CONFIG.theme.slotBg} p-4 flex flex-col items-center justify-center overflow-hidden`}>
                    ...
                    {/* Machine Border */}
                    <div className="relative bg-gradient-to-b from-[#1a2c38] to-[#0a1114] p-6 lg:p-10 rounded-3xl border-8 border-[#2f4553] shadow-[inset_0_20px_50px_rgba(0,0,0,0.8),0_20px_50px_rgba(0,0,0,0.5)]">
                        <div className="flex gap-4 sm:gap-6 bg-[#06090c] p-4 sm:p-6 rounded-2xl shadow-inner border-[4px] border-[#0f171c]">
                            ...
                            {reels.map((reelValueIdx, i) => (
                                <div key={i} className={`w-20 h-32 sm:w-28 sm:h-40 ${SLOTS_CONFIG.theme.reelBg} rounded-xl overflow-hidden relative border border-white/5 shadow-inner`}>
                                    ...
                                    <div className="flex flex-col items-center w-full h-full text-5xl sm:text-7xl absolute left-0 right-0">
                                        ...
                                        <AnimatePresence mode="popLayout">
                                            {spinningReels[i] ? (
                                                ...
                                                >
                                                    {/* Render a track of alternating symbols for the blur/spin effect */}
                                                    {[...SLOTS_CONFIG.symbols, ...SLOTS_CONFIG.symbols, ...SLOTS_CONFIG.symbols].map((sym, trackIdx) => (
                                                        <div key={trackIdx} className="flex justify-center items-center opacity-60 blur-[1px] h-20">
                                                            {sym.icon}
                                                        </div>
                                                    ))}
                                                </motion.div>
                                                ...
                                                >
                                                    {SLOTS_CONFIG.symbols[reelValueIdx]?.icon}
                                                </motion.div>
                                            )}
                                        </AnimatePresence>
                                    ...
                                    {gameState === 'WON' && !spinningReels[i] && (
                                        <motion.div
                                            ...
                                            className="absolute top-1/2 left-0 right-0 h-1 bg-[#00b9f0] -translate-y-1/2 z-20 shadow-[0_0_15px_#00b9f0]"
                                        />
                                    )}
                                </div>
                            ))}
